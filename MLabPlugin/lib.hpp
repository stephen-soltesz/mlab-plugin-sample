...
public:
    SampleTestClient(const std::string& hostname, MLabPluginAPI *mlab) :
        m_hostname(hostname) { m_api = mlab;};
    ...
    void        statusOLD(double tdiff, size_t amount);
    int         status(double tdiff, int amount);
